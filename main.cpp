...
skeleton3d get_default_skeleton() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        sk.shoulders[i] = float4(pos(i) * .35, .6, 0, 0);
        sk.elbows[i] = float4(pos(i) * .45, .25, .1, 0);
        sk.wrists[i] = float4(pos(i) * .3, -.05, -.2, 0);
        sk.hands[i] = float4(pos(i) * .3, -.05, -.2, 0);
        sk.hand_tips[i] = float4(pos(i) * .34, -.1, -.36, 0);
        sk.thumbs[i] = float4(pos(i) * .2, 0, -.25, 0);

        sk.hips[i] = float4(pos(i) * .2, 0, 0, 0);
        sk.knees[i] = float4(pos(i) * .17, -.5, -.1, 0);
        sk.ankles[i] = float4(pos(i) * .15, -.9, 0, 0);
        sk.feet[i] = float4(pos(i) * .15, -.9, -.2, 0);
    }
    return sk;
}
...
namespace vertex_buffer {
    ...
    void push_model(skeleton3d sk) {
        //        push_mtx(trans((sk.left_eye+sk.right_eye)/2.0f));
        //        push_mtx(scale(length(sk.left_eye-sk.right_eye)));
        //        push_test_cube();
        //        pop_mtx();
        //        pop_mtx();
       /* push_line(sk.left_shoulder, sk.right_shoulder);
        push_line(sk.left_hip, sk.right_hip);
        push_line((sk.left_shoulder + sk.right_shoulder) / 2.0f, (sk.left_hip + sk.right_hip) / 2.0f);*/
        ...
        for (int i = 0; i < 2; i++) {
            push_line(sk.spine_chest, sk.shoulders[i]);
            //push_line(sk.spine_chest, sk.clavicles[i]);
            //push_line(sk.clavicles[i], sk.shoulders[i]);
            push_line(sk.shoulders[i], sk.elbows[i]);
            push_line(sk.elbows[i], sk.wrists[i]);
            push_line(sk.wrists[i], sk.hands[i]);
            push_line(sk.hands[i], sk.hand_tips[i]);
            push_line(sk.hands[i], sk.thumbs[i]);

            push_line(sk.pelvis, sk.hips[i]);
            push_line(sk.hips[i], sk.knees[i]);
            push_line(sk.knees[i], sk.ankles[i]);
            push_line(sk.ankles[i], sk.feet[i]);
        }
    }
